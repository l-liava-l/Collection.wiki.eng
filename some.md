**some** возвращает `true`, если в исходной коллекции есть хотя бы один элемент, который подходит под заданной условие.

## Интерфейс

```js
some(opt_filterOrParams) { return boolean; }
```
...
`result` внутри фильтра ссылается на логическое значение операции.

### Примеры

```js
$C([1, 2, 3]).some(function (el) {
	return el > 1;
}) // true
