Кластерное API — это унифированный набор методов, который предоставляет вспомогательный функционал для работы с коллекциями. Всего существует несколько кластеров:

* collection — множество коллекций инкапсулированных в одном экземпляре Collection;
* filter — множество фильтров, которые можно «вызывать» в итераторах;
* context — множество указателей;
* namespace — множество пространств имён для работы с локальным хранилищем;
* var — множество переменных инкапсулированных в одном экземпляре Collection.

## Активность параметра

Если какой либо из кластерных параметров назначен активным, то он используется по умолчанию всегда.

### Ключевое слово active
