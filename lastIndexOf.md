**lastIndexOf** возвращает ключ первого с конца коллекции элемента, который равен заданному. Если исходная коллекция является экземпляром Map, то в качестве ответа будет возвращаться объект вида:
...
```js
lastIndexOf(searchElement, opt_startIndexOrParams) { return {value: ?} || ? || null; }
```
...
```js
$C([1, 2, 3, 2]).lastIndexOf(2) // 3
$C(new Map([[0, 1], [null, 2], [void 0, 2]])).lastIndexOf(2) // {value: undefined}
$C(new Set([1, 2, 3, 2])).lastIndexOf(2) // 3
```