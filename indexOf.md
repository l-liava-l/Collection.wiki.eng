**indexOf** возвращает ключ первого от начала коллекции элемента, который равен заданному. Если исходная коллекция является экземпляром Map, то в качестве ответа будет возвращаться объект вида:
