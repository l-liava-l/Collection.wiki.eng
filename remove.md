**remove** удаляет из исходной коллекции элементы, которые подходят под заданное условие, и возвращает массив объектов вида:

* `boolean` `result` — true, если элемент был удалён;
* `?` `key` — ключ удалённого элемента;
* `?` `value` — значение удалённого элемента.

Массив отсортирован в порядке удаления элементов.
Если удаление было с параметром `mult = false` или по указателю, то возвращается сам объект.

## Интерфейс

```js
remove(opt_filterOrParams) { return [] || {result: boolean, key: ?, value: ?}; }
```
...
$C([1, 2, 3, 4]).remove(function (el) {
	return el < 3;
}) // [{result: true, key: 0, value: 1}, {result: true, key: 1, value: 2}]

// Удаление значения по указателю
$C([1, 2, 3, 4]).remove(20) // {result: false, key: 20, value: undefined}
```